...
def puh():
    if len(list_1) == 1: return "Фраз должно быть больше одной"
        ...
        for i in list_1:
            ...
            # prpint(j for j in i if j in glasny)     # вывод генератора списка (гласные)
            # prpint(j for j in i if j not in glasny) # вывод с 'not' (согласные)
            # prpint(j for j in i if j in "")         # вывод: [], при пустом списке (см. кавычки)
