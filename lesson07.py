...
print(max(orbits, key = lambda x: x[0] * x[1] * (x[0] != x[1]))) # Вывод: (2.5, 10)
                                # "x[0] * x[1]" - площадь орбиты, без числа "Пи" можно обойтись
                                # "(x[0] != x[1])" - условие перемножения (сомножители не равны) 
                                ...
                                # 'key' - это не ключ, а внутренний аргумент функции,
                                # аналогичный аргументам 'end' у принта, либо 'set',
                                # 'end =' сообщает, чем будет заканчиваться принт, а
                                # 'key =' задаёт принцип выбора максимума функции "max()" (???)
                                # посмотреть характеристики функции "max()", её аргументы 

# 01:48:00
"""
Разбор ДЗ-07 
(см. файл "home07.py", папка "Python_Home")
"""
