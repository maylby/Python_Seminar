...
"""
orbits = [(1, 3), (2.5, 10), (7, 2), (6, 6), (4, 3)]
find_farthest_orbit = list(map(lambda x: x, orbits))

print(*find_farthest_orbit)
print(max(orbits, key = lambda x: x[0] * x[1] * (x[0] != x[1]))) # изящный способ решения
                                # "x[0] * x[1]" - вычисление площади орбиты без числа "Пи"
                                # "(x[0] != x[1])" - выполнение: "True", невыполнение: "False"
                                # умножение на False (ложь) равноценно умножению на 0 (ноль),
                                # т.е. такой кортеж удаляется из списка
